...
public class AppGlobalExceptionHandler {
	...
	public ModelAndView handleException(Exception exc, HttpServletRequest req) {
		...
		if(userInfo!=null) {
			if ("Type_STUDENT".equals((userInfo).getTypeName())) {
				view = "/student/error";
			} else if ("Type_CADRE_ADMIN".equals((userInfo).getTypeName())) {
				view = "/admin/error";
			} else if ("Type_PROF".equals((userInfo).getTypeName())) {
				view = "/prof/error";
			} else if ("Type_ADMIN".equals((userInfo).getTypeName())) {
				view = "/admin/error";
			} 
		}
