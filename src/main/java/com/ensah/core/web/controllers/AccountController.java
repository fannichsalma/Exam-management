...
@RequestMapping("/admin") // Très important car, dans Spring security les URL qui commencent par ADMIN
							// sont dédiées
							// à l'admin. Toutes les URL dédinies dans ce contTypeur définissent des
							// fonctionnalités
...
public class AccountController {
	...
	public String createAccountForm(@PathVariable int idPerson, Model model) {
		...
		//On ajoute la liste des Types dans le modele 
		model.addAttribute("TypeList", userService.getAllTypes());
	...
	public String addAccount(@ModelAttribute("accountModel") AccountModel accountModel, Model model) {
		...
		//Il suffit de passer l'id du Type et l'id de personne
		//à la couche service
		String password = userService.createUser(accountModel.getTypeId(), accountModel.getPersonId());
