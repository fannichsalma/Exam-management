...
public class RedirectionAfterAuthenticationSuccessHandler implements AuthenticationSuccessHandler {
	...
	protected String determineTargetUrl(final Authentication authentication) {
		...
		Map<String, String> TypeTargetUrlMap = new HashMap<>();

		//redirection selon le Type vers la page home
		TypeTargetUrlMap.put("ROLE_CADRE_ADMIN", "/cadreadmin/showHome");
		TypeTargetUrlMap.put("ROLE_PROF", "/prof/showHome");
		TypeTargetUrlMap.put("ROLE_ADMIN", "/admin/showAdminHome");
		...
		for (final GrantedAuthority grantedAuthority : authorities) {
			...
			if (TypeTargetUrlMap.containsKey(authorityName)) {
				return TypeTargetUrlMap.get(authorityName);
			}
		}
